...
def create_word_report(df, lab_results_df, filename, start_time, end_time):
    ...
    for col_name in ['C-01', 'C-02', 'C-03']:
        ...
        if reflux_tag in df.columns and top_flow_tag in df.columns:
            ...
            # Anomaly Detection
            tags_for_anomaly = [t for t in [reflux_tag, top_flow_tag, tags.get('reboiler_steam_flow')] if t in df.columns]
            ...
            if save_control_chart(tmp, f'{col_name}_reflux_ratio', out_png, title=f"{col_name} Reflux Ratio Control Chart", units="(dimensionless)", anomalies=anomalies_idx):
        ...
        doc.add_heading(f"5. {col_name} Time Series Forecasting", level=1)
        if tags.get('reflux_flow') in df.columns:
            forecast_df = time_series_forecast(df, tags['reflux_flow'], periods=24)
            if forecast_df is not None:
                ...
                ax.legend()
                fig.tight_layout()
                forecast_png = os.path.join(OUT_DIR, f"{col_name}_forecast.png")
                fig.savefig(forecast_png)
                plt.close(fig)
                doc.add_picture(forecast_png, width=Inches(6))
                doc.add_paragraph(f"Figure 3: ARIMA time series forecast for {tags['reflux_flow']}. The red line represents the forecast, with the shaded area showing the 95% confidence interval. This helps predict future process behavior.")
            else:
                doc.add_paragraph("Insufficient data to generate a time series forecast.")
